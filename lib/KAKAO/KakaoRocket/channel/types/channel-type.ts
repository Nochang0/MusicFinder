...
// 카카오톡 채널 로그인 토큰 구성요소
export interface Inside_LoginTokenData {
	access_token: string;	// 엑세스 토큰
	token_type: string;		// bearer
	expires_in: number;		// 유효기간
}

// 채널 유저정보 배열 구성요소
export interface Inside_UserListInfo {
	talk_user: {
	...
	profile_id: number;							// 내 채널 Id (카톡 채널유저 ID)
	encoded_profile_id: string;					// 채널 프로필 Id
	name: string;								// 유저 카톡 닉네임
	last_seen_log_id: number;					// 가장 나중에 읽은 유저 메세지 ID (관리자 기준, 0은 읽기 X)
	...
	last_log_id: number;						// 마지막 대화 메세지 ID (유저 기준)
	is_done: boolean;							// example: false (뭔지 모름)
	version: number;							// example: 타임스탬프 (뭔지 모름)
	last_log_send_at: number;					// example: 타임스탬프 (뭔지 모름)
	chat_label_ids: any[]						// example: [] (뭔지모름)
}

// 채널 유저정보 전체 배열 (유저가 없을 시 any[] 추가)
export interface Inside_UserList {
	has_next: boolean;
	items: Inside_UserInfo[];
}

// 유저 상세정보 요소
export interface Inside_UserInfo {
	talk_user: {
		id: number;								// User Id (카톡 유저 ID)
		nickname: string;						// 유저 카톡 닉네임
		status_message: string;					// 상태 메세지
